...
## What local ASR actually does to a short clip in Malayalam
...
Three things follow, and the console does all three:

1. **`VAANI_ASR_MODEL=medium`** trades speed for multilingual accuracy — the
   size is already an environment variable, no code change. Default stays
   `small` because ingestion is board-first and the operator should never wait
   on transcription.
2. **A low-confidence transcript says so.** Under 75% language probability, or
   under a 3-second clip, the detail panel prints
   `LOW-CONFIDENCE TRANSCRIPTION` and points at the recording.
